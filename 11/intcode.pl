...
run(Input, RemInput, Index, NewIndex, RelBase, NewRelBase, Output, NewOutput, ProgramHalted) :-
    get(Index, First),
    ...
    ;
        ...
        ;
            ...
            run(NInput, RemInput, NIndex, NewIndex, NRelBase, NewRelBase, NOut, NewOutput, ProgramHalted)
        )
    ).

run_to_halt(Program, Input, Output) :-
    list_to_mem(Program),
    run(Input, _, 0, _, 0, _, [], Output, true),
    clean_mem.
